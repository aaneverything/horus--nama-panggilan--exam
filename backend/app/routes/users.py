...
def register():
    ...
    if err == "missing_fields":
        return jsonify({"message": "Field wajib kurang"}), 400
    if err == "invalid_email":
        return jsonify({"message": "Email tidak valid"}), 400
    if err == "weak_password":
        return jsonify({"message": "Password lemah (>=8, huruf+angka)"}), 400
...
def get_user_detail(user_id: int):
    u = get_user(user_id)
    return {"id": int(u.id), "username": u.username, "email": u.email, "nama": u.nama}, 200
