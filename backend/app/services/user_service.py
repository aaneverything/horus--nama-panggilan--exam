...
def create_user(data: Mapping[str, Any]) -> Tuple[User | None, str | None]:
    ...
    if not username or not nama: return None, "missing_fields"
    if not validate_email(email): return None, "invalid_email"
    if not validate_password(password): return None, "weak_password"
...
def update_user(user_id: int, data: Mapping[str, Any]):
    ...
    if new_username:
        ...
        exists = db.session.scalar(select(User).where(and_(User.username == new_username, User.id != user_id)))
        if exists: return None, "username_taken"
        u.username = new_username
        
    if new_email:
        new_email = new_email.strip().lower()
        exists = db.session.scalar(select(User).where(and_(User.email == new_email, User.id != user_id)))
        if exists: return None, "email_taken"
        u.email = new_email
