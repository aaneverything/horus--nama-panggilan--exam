  ...
  <div class="max-w-md mx-auto bg-white border rounded p-6 shadow-sm">
    ...
    <form @submit.prevent="onSubmit" class="space-y-3">
      <div>
        ...
        <input v-model.trim="form.nama" type="text" class="w-full border rounded px-3 py-2" required />
        <p v-if="errs.nama" class="text-red-600 text-sm mt-1">{{ errs.nama }}</p>
      </div>

      <div>
        <label class="block mb-1">Email</label>
        <input v-model.trim="form.email" type="email" class="w-full border rounded px-3 py-2" required />
...
import { AuthAPI } from '../services/api' // pastikan ini call ke /api/users/register
...
function validate() {
  ...
  if (!form.nama) { errs.nama = 'Nama wajib diisi'; ok = false }

  if (!form.email) { errs.email = 'Email wajib diisi'; ok = false }
  else {
    const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    if (!re.test(form.email)) { errs.email = 'Format email tidak valid'; ok = false }
  }

  if (!form.username) { errs.username = 'Username wajib diisi'; ok = false }

  if (!form.password) { errs.password = 'Password wajib diisi'; ok = false }
  else {
...
const onSubmit = async () => {
  ...
  if (!validate()) { error.value = 'Field wajib kurang'; return }

  loading.value = true
  try {
    // kirim exactly field yg backend minta
    ...
    // tampilkan pesan dari backend kalau ada
